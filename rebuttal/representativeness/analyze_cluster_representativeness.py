...
PERMUTATIONS = 2000
FIT_ARTIFACT_NAMES = {
    "cluster_assignments_unlabeled.parquet",
    "cluster_assignments_sensitivity_unlabeled.parquet",
    "clustering_diagnostics.csv",
    "primary_model.npz",
    "computation_identity.json",
}
REPRESENTATIVENESS_SCOPE = "representativeness_root"
OUTPUT_SCOPE = "output_root"
FIT_SCOPE = "fit_root"
PATH_BASE_DEFINITIONS = {
    REPRESENTATIVENESS_SCOPE: "directory containing analyze_cluster_representativeness.py",
    OUTPUT_SCOPE: "directory containing fit/ and the score outputs",
    FIT_SCOPE: "directory containing fit_complete.json",
}
...
SCORE_OUTPUT_NAMES = {
    ...
    "matched_retrieval_sensitivity.csv",
...
def record(
    ...
    *,
    base: Path = ROOT,
    scope: str = REPRESENTATIVENESS_SCOPE,
) -> dict[str, object]:
    """Record an artifact with a checkout-portable, scope-relative path."""
    p = path.resolve(strict=True)
    resolved_base = base.resolve(strict=True)
    try:
        relative = p.relative_to(resolved_base)
    except ValueError as error:
        raise RuntimeError(f"Artifact escapes {scope}: {p}") from error
    if relative == Path(".") or ".." in relative.parts:
        raise RuntimeError(f"Invalid artifact path for {scope}: {relative}")
    return {
...
def resolve_record_path(
    ...
    *,
    ...
    """Resolve a scoped record while rejecting absolute paths and escapes."""
    scope = rec.get("path_scope")
    if not isinstance(scope, str) or scope not in bases:
        raise RuntimeError(f"Unknown artifact path scope: {scope!r}")
    raw = rec.get("path")
    if not isinstance(raw, str) or not raw:
        raise RuntimeError("Artifact path is absent or invalid")
    relative = Path(raw)
    if relative.is_absolute() or ".." in relative.parts:
        raise RuntimeError(f"Artifact path must be relative and contained: {raw}")
    base = bases[scope].resolve(strict=True)
    resolved = (base / relative).resolve(strict=False)
    try:
        resolved.relative_to(base)
    except ValueError as error:
        raise RuntimeError(f"Artifact path escapes {scope}: {raw}") from error
    return resolved
...
def fit_clusters(args: argparse.Namespace) -> int:
    ...
    identity={"version":"profile-cluster-fit-v1","path_base_definitions":{
        REPRESENTATIVENESS_SCOPE:PATH_BASE_DEFINITIONS[REPRESENTATIVENESS_SCOPE]},
        "design":record(DESIGN),"runner":record(Path(__file__)),
    ...
    write_json_new(fitdir/"fit_complete.json",{
        "status":"complete",
        "path_base_definition":{FIT_SCOPE:PATH_BASE_DEFINITIONS[FIT_SCOPE]},
        "artifacts":[record(p, base=fitdir, scope=FIT_SCOPE) for p in frozen],
    })
...
def verify_frozen_fit(out: Path) -> dict[str, object]:
    """Verify the completion marker and every hash-frozen fit artifact."""
    fitdir = out / "fit"
    completion_path = fitdir / "fit_complete.json"
    if not completion_path.is_file():
        raise RuntimeError(f"Frozen fit completion is absent: {completion_path}")
    completion = json.loads(completion_path.read_text())
    if completion.get("status") != "complete" or not isinstance(completion.get("artifacts"), list):
        raise RuntimeError("Frozen fit completion marker is invalid")
    recorded_names = {Path(rec.get("path", "")).name for rec in completion["artifacts"]}
    if recorded_names != FIT_ARTIFACT_NAMES or len(completion["artifacts"]) != len(FIT_ARTIFACT_NAMES):
        raise RuntimeError(f"Frozen fit artifact inventory drift: {sorted(recorded_names)}")
    for rec in completion["artifacts"]:
        if rec.get("path_scope") != FIT_SCOPE:
            raise RuntimeError(f"Frozen fit artifact has invalid scope: {rec.get('path_scope')!r}")
        p = resolve_record_path(rec, bases={FIT_SCOPE: fitdir})
        if p.parent != fitdir.resolve():
            raise RuntimeError(f"Frozen fit artifact escapes fit root: {p}")
        if record(p, base=fitdir, scope=FIT_SCOPE) != rec:
            raise RuntimeError(f"Frozen fit drift: {p}")
    return completion


def assert_score_root_pristine(out: Path) -> None:
    """Require a frozen fit-only root before the first scoring write."""
    unexpected = sorted(path.name for path in out.iterdir() if path.name != "fit")
    if unexpected:
        raise RuntimeError(f"Refusing partial/completed score root: {unexpected}")
    fitdir = out / "fit"
    if not fitdir.is_dir():
        raise RuntimeError(f"Frozen fit directory is absent: {fitdir}")


def assert_score_outputs_absent(out: Path) -> None:
    """Compatibility guard that comprehensively detects every scoring output."""
    existing = sorted(name for name in SCORE_OUTPUT_NAMES if (out / name).exists())
    partial = sorted(path.name for path in out.glob("*.partial"))
    if existing or partial:
        raise RuntimeError(
            f"Refusing to overwrite completed/partial score outputs: {existing + partial}"
        )
...
def score_selection(args: argparse.Namespace) -> int:
    ...
    verify_frozen_fit(out)
    assert_score_root_pristine(out)
    assert_score_outputs_absent(out)
    verify(SELECTED)
    ...
    for path in sorted(MANIFESTS.glob("matched_random_seed_*.parquet")):
        ...
        if (len(comp)!=3832 or len(set(comp))!=3832 or selected_set&set(comp)
                or set(comp)&eval_ids):
    ...
    provenance={"version":"profile-cluster-selection-v1","path_base_definitions":{
        REPRESENTATIVENESS_SCOPE:PATH_BASE_DEFINITIONS[REPRESENTATIVENESS_SCOPE],
        OUTPUT_SCOPE:PATH_BASE_DEFINITIONS[OUTPUT_SCOPE]},
        "design":record(DESIGN),"runner":record(Path(__file__)),
        "fit_completion":record(fitdir/"fit_complete.json",base=out,scope=OUTPUT_SCOPE),"scoring_inputs":[verify(SELECTED),record(EVALUATION)]+[record(p) for p in sorted(MANIFESTS.glob("matched_random_seed_*.parquet"))],
    ...
    write_json_new(out/"output_hashes.json",{
        "path_base_definition":{OUTPUT_SCOPE:PATH_BASE_DEFINITIONS[OUTPUT_SCOPE]},
        "files":[record(p,base=out,scope=OUTPUT_SCOPE) for p in outputs]
    })
